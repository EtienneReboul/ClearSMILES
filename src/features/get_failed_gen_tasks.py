"""
This scripts is used to find the jobs that have failed during the jobs array so they can be relaunched
"""
import os 
...
def main (search_pattern : str , output_filepath : str ,nb_task : int ) -> None :
    ...
    # check failed task
    failed_task_id_list= [i for i in range(nb_task) if not search_pattern.replace("*",str(i)) in finished_tasks]

    # write 
    if failed_task_id_list:
        nb_succesful_task= nb_task - len(failed_task_id_list)
        sucess_rate= nb_succesful_task/nb_task *100
        print(f"{nb_task-len(failed_task_id_list)}/{nb_task} tasks were successfully completed, so a {sucess_rate:.2f}% success rate")
